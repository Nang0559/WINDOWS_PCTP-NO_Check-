...
namespace PCTP.VIEWSTOCK.ViewForm
{
    public partial class FormTraHangNGNew : XtraForm
    {
        ...
        // ── THÊM field mới ──────────────────────────────────────────────
        private PhieuXuLyBatThuong _phieuQCHienTai;
        private LabelControl _lblKhoaQC;
        ...
        private List<LichSuQrCodeInfo> _lichSuQrFull = new List<LichSuQrCodeInfo>(); // cache toàn bộ, filter theo Stt khi chọn phiếu
                                                                                     // Thêm control mới trong BuildTabTuKho()
        private LookUpEdit _cboPhieuBatThuong;   // load từ _phieuLoiRepo.GetDanhSachDaDuyetChuaTra()
        ...
        {
            PreselectPhieuBatThuongId = preselectPhieuBatThuongId;
        }
        ...
        private void GridViewSlot_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            // ── SỬA: chỉ bật nút nếu đã chọn Slot VÀ QC đã duyệt OK ─────────────────
            _btnTra1.Enabled = _phieuQCHienTai != null && _phieuQCHienTai.DuDieuKienTraVeSX;
        }
        ...
        private void TraCuuLot(string lot)
        {
            RunWithLoadingSync(() =>
            {
                ...
                // ── THÊM: tra phiếu QC ngay đầu, dùng chung cho mọi nhánh bên dưới ──────
                _phieuQCHienTai = _traHangService.KiemTraLotChoTraVeSX(lot);
                ...
                // ── THÊM: hiển thị trạng thái phiếu QC ───────────────────────────────────
                if (_phieuQCHienTai == null)
                {
                    _lblInfo1.Text += "\n🔒 LOT này CHƯA có Phiếu Xử Lý Bất Thường — không thể trả về sản xuất " +
                                       "qua luồng hàng lỗi khách trả (cần ban hành phiếu ở Mốc 2 trước).";
                }
                else if (!_phieuQCHienTai.DuDieuKienTraVeSX)
                {
                    string tenBuoc;
                    switch (_phieuQCHienTai.TrangThai)
                    {
                        case TrangThaiXuLyBatThuong.ChoQC:
                            tenBuoc = "chờ QC định hướng";
                            break;
                        case TrangThaiXuLyBatThuong.QCDaDinhHuong:
                            tenBuoc = "đang chờ sản xuất xử lý";
                            break;
                        case TrangThaiXuLyBatThuong.ChoQCXacNhanCuoi:
                            tenBuoc = "chờ QC xác nhận cuối";
                            break;
                        default:
                            tenBuoc = _phieuQCHienTai.TrangThai.ToString();
                            break;
                    }
                    _lblInfo1.Text += $"\n🔒 Phiếu [{_phieuQCHienTai.SoPhieu}] {tenBuoc} — chưa thể trả về sản xuất.";
                }
                else
                {
                    _lblInfo1.Text += $"\n✅ Phiếu [{_phieuQCHienTai.SoPhieu}] đã được QC duyệt OK — đủ điều kiện trả về SX.";
                }
        ...
        private void BtnTra1_Click(object sender, EventArgs e)
        {
            ...
            // ── THÊM: chốt chặn cuối cùng ngay trước khi ghi DB — phòng trường hợp
            // dữ liệu QC vừa đổi giữa lúc TraCuuLot và lúc bấm nút (race condition) ────
            if (_phieuQCHienTai == null || !_phieuQCHienTai.DuDieuKienTraVeSX)
            {
                XtraMessageBox.Show(
                    "LOT này chưa được QC duyệt đủ điều kiện (QC_DA_DUYET + Kết luận OK) — " +
                    "không thể trả về sản xuất. Vui lòng kiểm tra lại ở Mốc 3 (QC Duyệt).",
                    "Chưa đủ điều kiện", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void LoadPhieuBatThuongDaDuyet()
        {
            var list = _phieuLoiRepo.GetDanhSachDaDuyetChuaTra();
            _cboPhieuBatThuong.Properties.DataSource = list;
            _cboPhieuBatThuong.Properties.DisplayMember = "SoPhieu";
            _cboPhieuBatThuong.Properties.ValueMember = "Id";

            if (PreselectPhieuBatThuongId.HasValue)
                _cboPhieuBatThuong.EditValue = PreselectPhieuBatThuongId.Value;
        }
