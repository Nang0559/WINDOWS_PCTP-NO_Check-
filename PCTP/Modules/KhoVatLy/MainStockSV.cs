...
namespace PCTP.VIEWSTOCK
{
    public partial class MainStockSV : DevExpress.XtraEditors.XtraForm
    {
        ...
        private readonly INhapKhoDashboardRepository _dashRepo = new NhapKhoDashboardRepository(new SQLPROVIDER());
        ...
        {
            string sqlTong = "SELECT ISNULL(SUM(SLCONLAI),0) FROM STOCKTP WHERE ISNULL(SLCONLAI,0) > 0";
            int tongStockTp = Convert.ToInt32(sqlProvider.ExecuteScalar(sqlProvider.B7R2_FCCdb, sqlTong));
        ...
        WHERE sl.PhieuStatus = 0 AND w.Name <> '{BulkImportConfig.WarehouseName}'";
            int tongRack = Convert.ToInt32(sqlProvider.ExecuteScalar(sqlProvider.B7R2_FCCdb, sqlRackThat));
        ...
        JOIN Warehouse w ON w.WarehouseId = r.WarehouseId
        WHERE sl.PhieuStatus = 0 AND w.Name = '{BulkImportConfig.WarehouseName}'";
            int tongA0 = Convert.ToInt32(sqlProvider.ExecuteScalar(sqlProvider.B7R2_FCCdb, sqlA0));

            int demLech = _dashRepo.DemLechDoiChieu();
        ...
        private void ShowDoiChieuLech()
        {
            using (var f = new FormNhapKhoTienTrinh(this))
                f.ShowDialog(this); // mở thẳng, hoặc tạo form riêng chỉ show bước 4
        }
        /// <summary>
        /// Kêt thúc
        /// </summary>
                ...
                if (rack.Bounds.Contains(logicalLocation))
                {
                    foreach (var slotLayout in rack.Slots)
                    {
                        if (slotLayout.Bounds.Contains(logicalLocation))
                        {
                            if (e.Button == MouseButtons.Left)
                            {
                                // ← THAY ĐỔI: click trái CHỈ chọn + hiện panel, KHÔNG mở dialog
                                _currentSelectedSlotData = slotLayout.SlotData;
                                pnlMain.Invalidate();
            ...
            foreach (var rack in _rackLayouts)
                if (rack.Bounds.Contains(logicalLocation))
                    foreach (var slotLayout in rack.Slots)
                        if (slotLayout.Bounds.Contains(logicalLocation))
                        {
                            OnSlotClicked(slotLayout.SlotData); // logic mở Export/Enter cũ, giữ nguyên
                            return;
                        }
        }

        // Context menu riêng cho Slot (thay vì chỉ đè lên click trái)
