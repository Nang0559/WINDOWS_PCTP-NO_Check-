...
{
    /// <summary>
    /// Read-only adapter for InspectionLog history.
    /// Transactional inspection writes remain owned by KhoCore/business modules.
    /// </summary>
...
ORDER BY MIN(CheckedAt) DESC";
            ...
            var table = _sql.LoadData(_sql.B7R2_FCCdbb, sql, parameters.ToArray());
