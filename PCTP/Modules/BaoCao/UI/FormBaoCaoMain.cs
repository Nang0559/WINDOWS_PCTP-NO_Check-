...
namespace PCTP.Modules.BaoCao.UI
{
    ...
    /// This form contains navigation only; business transactions remain in their owning modules.
    /// </summary>
    public sealed class FormBaoCaoMain : XtraForm
    {
        ...
        private void BuildUi()
        {
            ...
            var stock = new SimpleButton { Text = "Lịch sử kho / Tồn hiện tại", Dock = DockStyle.Fill };
            stock.Click += (s, e) => new FormBaoCaoStockHistory().ShowDialog(this);
            root.Controls.Add(stock, 0, 1);

            var quality = new SimpleButton { Text = "Lịch sử QC / Inspection", Dock = DockStyle.Fill };
            quality.Click += (s, e) => new FormBaoCaoQualityHistory().ShowDialog(this);
            root.Controls.Add(quality, 0, 2);
