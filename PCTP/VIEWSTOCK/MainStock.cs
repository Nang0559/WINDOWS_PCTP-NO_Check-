...
namespace PCTP.VIEWSTOCK
{
        ...
        public partial class MainStock : DevExpress.XtraEditors.XtraForm
        {
        ...
        private void OnSlotClicked(Slot slot)
        {
            ...
            try
            {
                if (slot.IsOccupied)
                {
                    ...
                    var exportForm = new ExportForm(slot, slot.RackName, slot.whname, this);
                    exportForm.ShowDialog(this);
                }
                else
                {
                    // Nếu ô đang trống -> Tự động mở Form Nhập Kho
                    var enterForm = new FormEnterItem(this);
        ...
        private void ShowExportFormFromCanvas(Slot slotData)
            {
                try
                {
                    var exportForm = new ExportForm(slotData, slotData.RackName, slotData.whname, this);
            ...
            private List<RackRenderInfo> LoadRackRenderInfosSync()
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    var slot = new Slot
                    {
                        SlotId = row["SlotId"] is DBNull ? 0 : Convert.ToInt32(row["SlotId"]),
                        whname = whName,
                        RackName = rackName,
                        Rackid = Convert.ToInt32(row["RackId"]),
                        ...
                        ImportDate = row["ImportDate"] is DBNull ? (DateTime?)null : Convert.ToDateTime(row["ImportDate"]),
                ...
                if (!string.IsNullOrEmpty(slot.ItemCode) && slot.Quantity > 0)
                    {
                        if (rackInfo.ItemSummary.TryGetValue(slot.ItemCode, out var s))
                            rackInfo.ItemSummary[slot.ItemCode] = (s.Item1 + 1, s.Item2 + slot.Quantity);
                        else
                            rackInfo.ItemSummary[slot.ItemCode] = (1, slot.Quantity);
                    }
                }
        ...
        private void btnEnterItem_Click(object sender, EventArgs e)
            {
                using (var form = new FormEnterItem(this))
                {
